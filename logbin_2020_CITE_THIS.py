...
def logbin(data, scale = 1., zeros = False, x_min = 1):
    """
    ...
    x_min: int
          minimum value of x -> we do not include bins with x < x_min
    ...
    y_PDF: array_like, 1 dimensional
          Array of normalised frequency counts within each bin. This represents
          a CONTINUOUS PDF based on our binning.
          Bins with a count of 0 will not be returned.
    y_counts: array_like, 1 dimensional
          Array of frequency counts within each bin. This represents
          the categorical frequencies for each bin.
          Bins with a count of 0 will not be returned.
    """
    if scale < 1:
    ...
    smax = np.max(data)
    if scale > 1:
        jmax = np.ceil((np.log(smax)-np.log(x_min))/np.log(scale))+1 #we added a +1 here because for some reason it's needed
        if zeros:
            binedges = x_min * (scale ** np.arange(jmax + 1))
            binedges[0] = 0
        else:
            binedges = x_min * (scale ** np.arange(0,jmax + 1)) #this should be 0,jmax+1, since then the first binedge is x_min
            # count = count[1:]
        binedges = np.unique(binedges.astype('uint64'))
        
        #omit low-value binedges
        #print("MIN BINEDGE =", binedges[0])
        #binedges = binedges[binedges >= x_min]
        #print("MIN BINEDGE NOW =", binedges[0])
        
        #check if x_min is sane
        if np.sum(count[0:x_min]) > 0:
            print(f"Logbinning omitted {np.sum(count[0:x_min])} counts cut off by x_min = {x_min}")
        #check if j_max is sane
        if np.sum(count[binedges[-1]:]) > 0:
            print(f"Logbinning omitted {np.sum(count[binedges[-1]:])} counts cut off by x_max = {binedges[-1]}")
        
        x = (binedges[:-1] * (binedges[1:])) ** 0.5
        y_PDF = np.zeros_like(x)
        y_counts = np.zeros_like(x)
        count = count.astype('float')
        for i in range(len(y_PDF)):
            y_PDF[i] = np.sum(count[binedges[i]:binedges[i+1]]/(binedges[i+1] - binedges[i]))
            y_counts[i] = np.sum(count[binedges[i]:binedges[i+1]])
            # print(binedges[i],binedges[i+1])
        # print(smax,jmax,binedges,x)
        # print(x,y)
    else:
        x = np.nonzero(count)[0]
        y_PDF = count[count != 0].astype('float')
        if zeros != True and x[0] == 0:
            x = x[1:]
            y_PDF = y_PDF[1:]
    y_PDF /= tot
    #y_counts /= tot
    # We want the bins - but we dont want nonzero bins for plotting. Hence we will return both zero-including data
    # (as it would be cheating to omit zeros BEFORE goodness of fit testing) AND zero-omitting data
    
    #x_nonzero = x[y!=0]
    #y_nonzero = y[y!=0]
    return x,y_PDF, y_counts, binedges
